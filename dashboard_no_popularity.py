...
def load_ai_model():
    ...
    try:
        model.load_model("models/xgboost_box_office_model_without_popularity.json")
...
with col1:
    ...
    # Uses st.multiselect to allow multiple genres
...
# Loop through the list of selected genres and encode each one
...
if st.button("🚀 RUN PREDICTIVE ANALYSIS & RISK SIMULATION", type="primary", use_container_width=True):
    with st.status("Initializing AI Systems...", expanded=True) as status:
        try:
            ...
            # REMOVED: Log transformation handling
